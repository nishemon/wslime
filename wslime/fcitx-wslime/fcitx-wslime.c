...
static INPUT_RETURN_VALUE fcitx_wslime_sync(FcitxWSLime* wslime);
...
DEBUG(wslime->fd, buff);
    wslime_inputkey(wslime->con, sym, state);
    wslime->support_client_preedit = context->contextCaps & CAPACITY_PREEDIT;
    return fcitx_wslime_sync(wslime);
}

static INPUT_RETURN_VALUE fcitx_wslime_sync(FcitxWSLime* wslime)
{
    wslime_composition* composition = wslime_read(wslime->con);
   if (composition == NULL) {
       return IRV_TO_PROCESS;
    }
    FcitxInputState *input = FcitxInstanceGetInputState(wslime->owner);
